...
def _choose_station_key(df: pd.DataFrame) -> str:
    candidates = [
        "station_name","station","nom_station","Nom_station","Station","NomStation",
        "station_id","code_station","Code_Station","no_station","No_Station"
    ]
    for c in candidates:
        if c in df.columns:
            return c
...
def build_features(
    ...
    keep = [c for c in ["datetime","station_id","station_name","pollutant","value"] if c in x.columns]
    x = x[keep]
    ...
    if df_weather is not None and len(df_weather) > 0 and "datetime" in df_weather.columns:
        w = df_weather.copy()
    ...
    if median_step >= 24*3600:
        x["roll_7"] = x.groupby(gkey)["value"].transform(lambda s: s.shift(1).rolling(7).mean())
    else:
        x["roll_24"] = x.groupby(gkey)["value"].transform(lambda s: s.shift(1).rolling(24).mean())
    ...
    features = [c for c in x.columns if c not in ("datetime","station_id","station_name","pollutant","value","target")]
    return x, features
...
def build_features_daily_iqa(df_daily: pd.DataFrame, lags=(1,2,3,7,14)):
    ...
    # date -> datetime
    for cand in ["datetime","date","Date","DATE","date_heure","Date_Heure","jour","Jour","JOUR"]:
    ...
    # value -> IQA
    value_candidates = ["value","iqa","IQA","indice","Indice","iqa_value","valeur_iqa","Valeur","valeur"]
