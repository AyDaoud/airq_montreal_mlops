...
**Plan defect found during execution, fixed in `3b`:** Task 3 built and unit-tested `ModelGauges` but no task wired it into the application — `ModelGauges` appeared in this plan only inside test code. `airq_mase_*` and `airq_data_freshness_days` would therefore never have reached `/metrics`, and the dashboard's two headline panels would have queried non-existent series while the SQLite panels rendered normally and hid the problem. Resolved by adding `ModelMetricsCollector`, registered on the Prometheus registry at module import and computing at scrape time. The lesson generalises: a plan that *builds* a component must name the task that *calls* it.
